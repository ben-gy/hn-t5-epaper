...
#include "glyph.h"
// Derived from LilyGo-EPD47 font.c (GPL-3.0). Hardware and zlib paths removed:
// bitmaps are stored uncompressed and drawing goes only to a caller's buffer.
#ifndef IRAM_ATTR
#define IRAM_ATTR
#endif
typedef unsigned long uLongf;
typedef unsigned char Bytef;
static int uncompress(Bytef *d, uLongf *dl, const Bytef *sv, uLongf sl) {   // compressed fonts unsupported
    (void)d; (void)dl; (void)sv; (void)sl; return -1;
}
#include <assert.h>
#include <stdlib.h>
...
static void glyph_hline(int32_t x, int32_t y, int32_t length, uint8_t color, uint8_t *fb) {
    for (int32_t i = 0; i < length; i++) {
        int32_t px = x + i;
        if (px < 0 || px >= epd_fb_width || y < 0 || y >= epd_fb_height) continue;
        uint8_t *p = &fb[(size_t)y * (epd_fb_width / 2) + px / 2];
        if (px & 1) *p = (*p & 0x0F) | (color & 0xF0);
                ...
                const FontProperties *properties)
{
    ...
    *cursor_y += local_cursor_y - cursor_y_init;
    ...
    if (framebuffer == NULL)
    {
        ...
        (void)area; (void)mode;   /* no direct panel path */
        free(buffer);
    }
}
