...
// The gauge does not tolerate a repeated start after the command byte on this
// bus (the read times out with ESP_ERR_TIMEOUT), and it needs a moment between
// transactions: write the register, release the bus, settle, then read.
static bool gaugeRead(uint8_t reg, uint16_t &v) {
    for (int attempt = 0; attempt < 3; attempt++) {
        ...
        if (Wire.endTransmission(true) == 0) {
            ...
            if (Wire.requestFrom((uint8_t)0x55, (uint8_t)2) == 2) {
                v = Wire.read();
...
// The status line asks on every header draw; the cell does not move that fast.
struct GaugeCache { uint32_t at = 0; bool valid = false; uint16_t v = 0; };
static bool gaugeCached(uint8_t reg, GaugeCache &c, uint16_t &out) {
    uint32_t now = millis();
    if (!c.at || now - c.at > 30000) {
        c.valid = gaugeRead(reg, c.v);
        c.at = now ? now : 1;
    }
    out = c.v;
    return c.valid;
...
uint32_t batteryMilliVolts() {
    static GaugeCache c;
    uint16_t v;
    return gaugeCached(0x08, c, v) ? v : 0;
}
int batteryPercent() {
    static GaugeCache c;
    uint16_t s;
    if (!gaugeCached(0x2C, c, s)) return -1;
    return s > 100 ? 100 : (int)s;
}
